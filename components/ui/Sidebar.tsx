    ...
    <aside className="flex flex-col h-full flex-shrink-0 transition-all duration-300 border-r" style={{ width: collapsed ? 60 : 228, background: "#0d1f40", borderColor: "rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex items-center gap-2.5 px-3.5 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
        ...
        {!collapsed && (
          <div>
            ...
            <p className="text-white/40 text-[9px] mt-0.5 leading-none font-medium uppercase tracking-wider">Kabupaten Ciamis</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 py-2 overflow-y-auto">
        {nav.map(({ key, label, icon: Icon }) => (
          ...
          >
            {page === key && <span className="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-r" style={{ background: "#E3C265" }} />}
      ...
      <div className="border-t p-3" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
        {!collapsed ? (
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0" style={{ background: "linear-gradient(135deg,#0B4D3A,#1E7A4C)" }}>
